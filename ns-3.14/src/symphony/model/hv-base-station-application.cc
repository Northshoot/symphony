...
HvBaseStation::HvBaseStation (TosNodeContainer nodes)
{
	...
	item_len = 0; input_shift = 0; cur_state_num = 0; count_adr = 0; count_shift = 0; node_pairs_number = (int) nodes.GetN();
	sum_adres = 1; sum_shift = 1; k_shift = 1;
...
void HvBaseStation::ReceiveHyperVector(uint16_t size, void * buffer)
{
	...
	if (check_sum < 0.20) { // Discard input vector, it is already in the buffer
        for (int i=0;i<a;i++) input_vector[i] = 0;
		return;
    } else { // Process new vector
        ...
        //If it is sensor
        if (input_id < node_pairs_number/2 ) { //  sensors id less than number of nodes / 2
            ...
            for (int i=0; i<a;i++) {
            	...
            	if (sen_state[cur_state_num].init_flag == 1){
                	...
                	if (i == a-1) { sen_state[cur_state_num].init_flag = 1;}
                }
            }
        	...
        	input_shift = 0; input_id = 0; count_adr = 0; count_shift = 0;
        	sum_adres = 1; sum_shift = 1; k_shift = 1;

        // if vector from actuator
        } else if (input_id >= node_pairs_number/2 ) {
...
void HvBaseStation::SendOutputVector(int *output_vector)
{
    ...
    io_server->SendData(result); // Notifies I/O Clients

    //Sends the message to the actuator nodes
