 ...
 */
#ifndef TOS_NS_RADIO_H
#define TOS_NS_RADIO_H
...
/**
 * \brief listen to events coming from ns3::TosNsRadio.
 */
class TosNsRadioTransmissionListener {
public:
  TosNsRadioTransmissionListener ();
  virtual ~TosNsRadioTransmissionListener ();

  /**
   * \param snr the snr of the cts
   * \param txMode the txMode of the cts
   *
   * ns3::TosNsRadio received an expected CTS within
   * CtsTimeout.
   */
  virtual void GotCts (double snr, WifiMode txMode) = 0;
  /**
   * ns3::TosNsRadio did not receive an expected CTS
   * within CtsTimeout.
   */
   ...
   *
   * ns3::TosNsRadio received an expected ACL within
  ...
  /**
   * ns3::TosNsRadio did not receive an expected ACK within
   * AckTimeout.
   */
   ...
   *
   * Invoked when ns3::TosNsRadio receives a block ack frame.
   ...
   * queue that intends to be notified by TosNsRadio of reception
   * of a block ack must redefine this function.
   */
  virtual void GotBlockAck (const CtrlBAckResponseHeader *blockAck, Mac48Address source);
  /**
   * ns3::TosNsRadio did not receive an expected BLOCK_ACK within
  ...
  /**
   * Invoked when ns3::TosNsRadio wants to start a new transmission
   * as configured by TosNsRadioTransmissionParameters::EnableNextData.
   * The listener is expected to call again TosNsRadio::StartTransmission
 ...
 * ns3::Dcf methods.
 */
class TosNsRadioDcfListener {
public:
  TosNsRadioDcfListener ();
  virtual ~TosNsRadioDcfListener ();
  /**
 ...
 */
class TosNsRadioBlockAckEventListener {
public:
  TosNsRadioBlockAckEventListener ();
  virtual ~TosNsRadioBlockAckEventListener ();
  /**
 ...
 *
 * The ns3::TosNsRadio::StartTransmission method expects
 * an instance of this class to describe how the packet
 * should be transmitted.
 */
class TosNsRadioTransmissionParameters {
public:
  TosNsRadioTransmissionParameters ();

  /**
   * Wait ACKTimeout for an ACK. If we get an ACK
   * on time, call TosNsRadioTransmissionListener::GotAck.
   * Call TosNsRadioTransmissionListener::MissedAck otherwise.
   */
  void EnableAck (void);
  /**
   *   - wait PIFS after end-of-tx. If idle, call
   *     TosNsRadioTransmissionListener::MissedAck.
   *   - if busy at end-of-tx+PIFS, wait end-of-rx
   *   - if Ack ok at end-of-rx, call
   *     TosNsRadioTransmissionListener::GotAck.
   ...
   * CTS on time, call TosNsRadioTransmissionListener::GotCts
   * and send data. Otherwise, call TosNsRadioTransmissionListener::MissedCts
   ...
   * TosNsRadioTransmissionListener::StartNext at the end of
   ...
   * value is automatically calculated by the TosNsRadio before
...
private:
  friend std::ostream &operator << (std::ostream &os, const TosNsRadioTransmissionParameters &params);
  uint32_t m_nextSize;
  enum {
    ACK_NONE,
    ACK_NORMAL,
 ...
 */
class TosNsRadio : public Object {
public:
  typedef Callback<void, Ptr<Packet>, const WifiMacHeader*> TosNsRadioRxCallback;
   ...
   *        and outgoing packet.
   */
  void RegisterDcfListener (TosNsRadioDcfListener *listener);
  ...
  Time CalculateTransmissionTime (Ptr<const Packet> packet,
                                  ...
                                  const TosNsRadioTransmissionParameters& parameters) const;
  ...
  void StartTransmission (Ptr<const Packet> packet, 
                          const WifiMacHeader* hdr, 
                          TosNsRadioTransmissionParameters parameters,
   ...
   *
   * This method is typically invoked by the PhyTosNsRadioListener to notify
   ...
   * Response frame. At this point TosNsRadio must allocate buffers to
   ...
   */
  void RegisterBlockAckListenerForAc (enum AcIndex ac, TosNsRadioBlockAckEventListener *listener);
private:
  ...
  void SetupPhyTosNsRadioListener (Ptr<WifiPhy> phy);

  Ptr<WifiPhy> m_phy;
  Ptr<WifiRemoteStationManager> m_stationManager;
  TosNsRadioRxCallback m_rxCallback;
  typedef std::vector<TosNsRadioDcfListener *>::const_iterator DcfListenersCI;
  typedef std::vector<TosNsRadioDcfListener *> DcfListeners;
  ...
  TosNsRadioTransmissionParameters m_txParams;
  TosNsRadioTransmissionListener *m_listener;
  Mac48Address m_self;
  ...
  class PhyTosNsRadioListener *m_phyTosNsRadioListener;
  ...
  typedef std::map<AcIndex, TosNsRadioBlockAckEventListener*> QueueListeners;
  QueueListeners m_edcaListeners;
};

} // namespace ns3

#endif /* TOS_NS_RADIO_H */
