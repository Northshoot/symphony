 ...
 */
#include "ns3/tos-mobility-helper.h"
...
NS_LOG_COMPONENT_DEFINE ("TosMobilityHelper");
...
void
TosMobilityHelper::SetPositionAllocator (Ptr<PositionAllocator> allocator)
{
  m_position = allocator;
}

void
TosMobilityHelper::SetPositionAllocator (std::string type,
...
void 
TosMobilityHelper::SetMobilityModel (std::string type,
                                  std::string n1, const AttributeValue &v1,
                                  std::string n2, const AttributeValue &v2,
                                  std::string n3, const AttributeValue &v3,
                                  std::string n4, const AttributeValue &v4,
...
void 
TosMobilityHelper::PushReferenceMobilityModel (Ptr<Object> reference)
{
...
void 
TosMobilityHelper::PushReferenceMobilityModel (std::string referenceName)
{
...
void 
TosMobilityHelper::PopReferenceMobilityModel (void)
{
...
std::string 
TosMobilityHelper::GetMobilityModelType (void) const
{
...
void
TosMobilityHelper::Install (std::string nodeName) const
{
  Ptr<TosNode> node = Names::Find<TosNode> (nodeName);
  Install (node);
}
void 
TosMobilityHelper::Install (TosNodeContainer c) const
{
...
void 
TosMobilityHelper::InstallAll (void)
{
  Install (TosNodeContainer::GetGlobal ());
}
static double
DoRound (double v)
{
...
void
TosMobilityHelper::CourseChanged (std::ostream *os, Ptr<const MobilityModel> mobility)
{
  Ptr<TosNode> node = mobility->GetObject<TosNode> ();
...
void 
TosMobilityHelper::EnableAscii (std::ostream &os, uint32_t nodeid)
{
  std::ostringstream oss;
  oss << "/TosNodeList/" << nodeid << "/$ns3::MobilityModel/CourseChange";
  Config::ConnectWithoutContext (oss.str (), 
                                 MakeBoundCallback (&TosMobilityHelper::CourseChanged, &os));
}
void 
TosMobilityHelper::EnableAscii (std::ostream &os, TosNodeContainer n)
{
  for (TosNodeContainer::Iterator i = n.Begin (); i != n.End (); ++i)
    {
      EnableAscii (os, (*i)->GetId ());
    }
}
void 
TosMobilityHelper::EnableAsciiAll (std::ostream &os)
{
