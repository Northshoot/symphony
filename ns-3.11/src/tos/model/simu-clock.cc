...
SimuClock::SimuClock(PRECISION p,TIMEDRIFT t, Callback<uint32_t,uint32_t> tf)
...
SimuClock::timerFired(){
	...
	if(type == NONE){
		tick_event =
				Simulator::Schedule(tickTime, &SimuClock::timerFired, this);
		count++;
		callBack(0);
	} else if (type == STATIC) {
		//TODO:
		std::cout<<"SimuClock::timerFired() STATIC not implemented!"<<std::endl;
		exit(1);
	} else if (type == EXPONENTIAL) {
		//TODO:
		std::cout<<"SimuClock::timerFired() EXPONENTIAL not implemented!"<<std::endl;
		exit(1);
	}
}
