...
/***************************************************************
 *         Listener for PHY events. Forwards to TosMacLow
 ***************************************************************/
...
void
TosMacLow::SetWifiRemoteStationManager (Ptr<WifiRemoteStationManager> manager)
{
  ...
  if (hdr.IsRts ())
      ...
      if (isPrevNavZero
          && hdr.GetAddr1 () == m_self)
        {
          ...
          m_sendCtsEvent = Simulator::Schedule (GetSifs (),
                                                &TosMacLow::SendCtsAfterRts, this,
                                                hdr.GetAddr2 (),
                                                hdr.GetDuration (),
                                                txMode,
                                                rxSnr);
           ...
           && m_currentPacket != 0)
    {
      ...
      m_sendDataEvent = Simulator::Schedule (GetSifs (),
                                             &TosMacLow::SendDataAfterCts, this,
                                             hdr.GetAddr1 (),
                                             hdr.GetDuration (),
                                             txMode);
    }
      ...
      if (m_txParams.HasNextPacket ())
        {
          m_waitSifsEvent = Simulator::Schedule (GetSifs (),
                                                 &TosMacLow::WaitSifsAfterEndTx, this);
        }
    }
              ...
              if ((*it).second.first.IsImmediateBlockAck ())
                {
                  ...
                  m_sendAckEvent = Simulator::Schedule (GetSifs (),
                                                        &TosMacLow::SendBlockAckAfterBlockAckRequest, this,
                                                        blockAckReq,
                                                        hdr.GetAddr2 (),
                                                        hdr.GetDuration (),
                                                        txMode);
          ...
          if (hdr.IsQosAck ())
            {
              ...
              NS_ASSERT (m_sendAckEvent.IsExpired ());
              m_sendAckEvent = Simulator::Schedule (GetSifs (),
                                                    &TosMacLow::SendAckAfterData, this,
                                                    hdr.GetAddr2 (),
                                                    hdr.GetDuration (),
                                                    txMode,
                                                    rxSnr);
...
uint32_t
TosMacLow::GetAckSize (void) const
{
  WifiMacHeader ack;
...
uint32_t
TosMacLow::GetBlockAckSize (enum BlockAckType type) const
{
  WifiMacHeader hdr;
...
uint32_t
TosMacLow::GetRtsSize (void) const
{
  WifiMacHeader rts;
...
Time
TosMacLow::GetAckDuration (Mac48Address to, WifiMode dataTxMode) const
{
  WifiMode ackMode = GetAckTxModeForData (to, dataTxMode);
  return m_phy->CalculateTxDuration (GetAckSize (), ackMode, WIFI_PREAMBLE_LONG);
}
Time
TosMacLow::GetBlockAckDuration (Mac48Address to, WifiMode blockAckReqTxMode, enum BlockAckType type) const
{
  /*
...
WifiMode
TosMacLow::GetRtsTxMode (Ptr<const Packet> packet, const WifiMacHeader *hdr) const
{
  Mac48Address to = hdr->GetAddr1 ();
  return m_stationManager->GetRtsMode (to, hdr, packet);
}
WifiMode
TosMacLow::GetDataTxMode (Ptr<const Packet> packet, const WifiMacHeader *hdr) const
{
  Mac48Address to = hdr->GetAddr1 ();
  WifiMacTrailer fcs;
...
WifiMode
TosMacLow::GetCtsTxModeForRts (Mac48Address to, WifiMode rtsTxMode) const
{
  return m_stationManager->GetCtsMode (to, rtsTxMode);
}
WifiMode
TosMacLow::GetAckTxModeForData (Mac48Address to, WifiMode dataTxMode) const
{
...
void
TosMacLow::NotifyNav (const WifiMacHeader &hdr, WifiMode txMode, WifiPreamble preamble)
{
      ...
      if (hdr.IsRts () && navUpdated)
        {
          ...
          m_navCounterResetCtsMissed = Simulator::Schedule (navCounterResetCtsMissedDelay,
                                                            &TosMacLow::NavCounterResetCtsMissed, this,
                                                            Simulator::Now ());
        }
    }
}
...
void
TosMacLow::NavCounterResetCtsMissed (Time rtsEndRxTime)
{
...
void
TosMacLow::DoNavResetNow (Time duration)
{
...
bool
TosMacLow::DoNavStartNow (Time duration)
{
  for (DcfListenersCI i = m_dcfListeners.begin (); i != m_dcfListeners.end (); i++)
    {
      (*i)->NavStart (duration);
...
void
TosMacLow::NotifyAckTimeoutStartNow (Time duration)
{
  for (DcfListenersCI i = m_dcfListeners.begin (); i != m_dcfListeners.end (); i++)
    {
...
void
TosMacLow::NotifyAckTimeoutResetNow ()
{
  for (DcfListenersCI i = m_dcfListeners.begin (); i != m_dcfListeners.end (); i++)
    {
      (*i)->AckTimeoutReset ();
...
void
TosMacLow::NotifyCtsTimeoutStartNow (Time duration)
{
  for (DcfListenersCI i = m_dcfListeners.begin (); i != m_dcfListeners.end (); i++)
    {
...
void
TosMacLow::NotifyCtsTimeoutResetNow ()
{
  for (DcfListenersCI i = m_dcfListeners.begin (); i != m_dcfListeners.end (); i++)
    {
      (*i)->CtsTimeoutReset ();
...
void
TosMacLow::NormalAckTimeout (void)
{
  NS_LOG_FUNCTION (this);
...
void
TosMacLow::BlockAckTimeout (void)
{
  NS_LOG_FUNCTION (this);
...
void
TosMacLow::SendRtsForPacket (void)
{
  NS_LOG_FUNCTION (this);
  ...
  m_ctsTimeoutEvent = Simulator::Schedule (timerDelay, &TosMacLow::CtsTimeout, this);
  ...
  if (m_txParams.MustWaitNormalAck ())
    {
      ...
      m_normalAckTimeoutEvent = Simulator::Schedule (timerDelay, &TosMacLow::NormalAckTimeout, this);
    }
  else if (m_txParams.MustWaitFastAck ())
    {
      ...
      NotifyAckTimeoutStartNow (timerDelay);
      m_fastAckTimeoutEvent = Simulator::Schedule (timerDelay, &TosMacLow::FastAckTimeout, this);
    }
  else if (m_txParams.MustWaitSuperFastAck ())
    {
      ...
      m_superFastAckTimeoutEvent = Simulator::Schedule (timerDelay,
                                                        &TosMacLow::SuperFastAckTimeout, this);
    }
  else if (m_txParams.MustWaitBasicBlockAck ())
    {
      ...
      m_blockAckTimeoutEvent = Simulator::Schedule (timerDelay, &TosMacLow::BlockAckTimeout, this);
    }
...
bool
TosMacLow::IsNavZero (void) const
{
...
void
TosMacLow::SendCtsAfterRts (Mac48Address source, Time duration, WifiMode rtsTxMode, double rtsSnr)
{
...
void
TosMacLow::SendDataAfterCts (Mac48Address source, Time duration, WifiMode txMode)
{
  NS_LOG_FUNCTION (this);
...
void
TosMacLow::FastAckFailedTimeout (void)
{
  NS_LOG_FUNCTION (this);
  MacLowTransmissionListener *listener = m_listener;
  m_listener = 0;
...
void
TosMacLow::SendAckAfterData (Mac48Address source, Time duration, WifiMode dataTxMode, double dataSnr)
{
  NS_LOG_FUNCTION (this);
...
bool
TosMacLow::StoreMpduIfNeeded (Ptr<Packet> packet, WifiMacHeader hdr)
{
  AgreementsI it = m_bAckAgreements.find (std::make_pair (hdr.GetAddr2 (), hdr.GetQosTid ()));
  if (it != m_bAckAgreements.end ())
    {
...
void
TosMacLow::CreateBlockAckAgreement (const MgtAddBaResponseHeader *respHdr, Mac48Address originator,
...
void
TosMacLow::DestroyBlockAckAgreement (Mac48Address originator, uint8_t tid)
{
...
void
TosMacLow::RxCompleteBufferedPacketsWithSmallerSequence (uint16_t seq, Mac48Address originator, uint8_t tid)
{
  AgreementsI it = m_bAckAgreements.find (std::make_pair (originator, tid));
  if (it != m_bAckAgreements.end ())
    {
...
void
TosMacLow::RxCompleteBufferedPacketsUntilFirstLost (Mac48Address originator, uint8_t tid)
{
  AgreementsI it = m_bAckAgreements.find (std::make_pair (originator, tid));
  if (it != m_bAckAgreements.end ())
    {
...
void
TosMacLow::SendBlockAckResponse (const CtrlBAckResponseHeader* blockAck, Mac48Address originator, bool immediate,
...
void
TosMacLow::SendBlockAckAfterBlockAckRequest (const CtrlBAckRequestHeader reqHdr, Mac48Address originator,
                                          Time duration, WifiMode blockAckReqTxMode)
{
  NS_LOG_FUNCTION (this);
  CtrlBAckResponseHeader blockAck;
...
//void
//TosMacLow::ResetBlockAckInactivityTimerIfNeeded (BlockAckAgreement &agreement)
//{
...
//void
//TosMacLow::RegisterBlockAckListenerForAc (enum AcIndex ac, MacLowBlockAckEventListener *listener)
//{
