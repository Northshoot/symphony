...
#ifndef TOS_MOBILITY_HELPER_H
#define TOS_MOBILITY_HELPER_H
...
public:
   ...
   * \param v9 the value of the attribute to set in the mobility model.
   *
   * Calls to TosMobilityHelper::Install will create an instance of a matching
   ...
   *
   * If this stack is not empty when TosMobilityHelper::Install
   ...
   * type of which was set with TosMobilityHelper::SetMobilityModel), aggregates
   * it to the provided node, and sets an initial position based on the current
   * position allocator (set through TosMobilityHelper::SetPositionAllocator).
   *
   * \param node The node to "layout."
   */
  void Install (Ptr<TosNode> node) const;
  /**
   * \brief "Layout" a single node according to the current position allocator type.
   *
   * This method creates an instance of a ns3::MobilityModel subclass (the 
   ...
   * TosMobilityHelper::SetMobilityModel), aggregates it to the node, and sets an
   * initial position based on the current position allocator (set through 
   * TosMobilityHelper::SetPositionAllocator).
   *
   * \param container The set of nodes to layout.
   */
  void Install (TosNodeContainer container) const;

  /**
   * Perform the work of TosMobilityHelper::Install on _all_ nodes which
   ...
   */
  static void EnableAscii (std::ostream &os, TosNodeContainer n);
  /**
   * \param os output stream
   *
   ...
   */
  int64_t AssignStreams (TosNodeContainer c, int64_t stream);
